...
export default {
  ...
  run: async (client, m, args, usedPrefix, command, text) => {
    try {
      ...
      await client.sendMessage(m.chat, { stickerPack: { name: packName, publisher: author?.name || author?.username || `@${name}`, description: 'ɴᴇᴋᴏ 🧠 Wᴀʙᴏᴛ', cover, stickers: stickerResults } }, { quoted: m });
      await m.react('✔️');
    } catch (e) {
      await m.react('✖️');
